...
export function dragAndDrop(node: HTMLElement, { onReorder, index }: { onReorder: (fromIndex: number, toIndex: number) => void; index: number }) {
	let state: DragState = {
		isDragging: false,
		startY: 0,
		currentY: 0,
	...
	return {
		update({ onReorder: newOnReorder, index: newIndex }: { onReorder: (fromIndex: number, toIndex: number) => void; index: number }) {
